...
(function() {
  var AreaSummary, Confirmbox, CustomInfoWindow, CustomMap, Modalbox, Playlist, TemplatesLoader,
  ...
  Playlist = (function() {
    ...
    function Playlist(map) {
      ...
      $('html').keydown(function(e) {
        if (e.keyCode === 37) {
          return _this.shiftCurrentMarkerToLeft();
        } else if (e.keyCode === 39) {
          return _this.shiftCurrentMarkerToRight();
        }
      });
    ...
    Playlist.prototype.addMarker = function(markerGroup, markerType, markerID) {
      var iconPath, item, mark, marker, markerTitle, _i, _len, _ref,
        _this = this;
      if (this.markers[markerID] != null) {
        return;
      }
      marker = null;
      _ref = this.map.gMarker[markerGroup]['marker_types'][markerType]['markers'];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        mark = _ref[_i];
        if (mark.__gm_id === markerID) {
          marker = mark;
        }
      }
      if ((marker != null) && (marker['data_translation'] != null) && (marker['data_translation'][window.LANG]['title'] != null) && marker['data_translation'][window.LANG]['title'] !== "") {
        markerTitle = marker['data_translation'][window.LANG]['title'];
      } else {
        markerTitle = this.map.gMarker[markerGroup]['marker_types'][markerType]['data_translation'][window.LANG]['name'];
      }
      if (markerTitle.length >= 15) {
        markerTitle = markerTitle.substr(0, 12) + '...';
      }
      iconPath = this.map.markersImages[markerType].url;
      item = $('<li class="type-menu-item" data-markerID="' + markerID + '"><a class="marker-type-link"><img src="' + iconPath + '" /><span>' + markerTitle + '</span></a></li>');
      item.click(function(e) {
        marker = $(e.currentTarget);
        return _this.setSelectedMarker(marker);
      });
      this.markers[markerID] = marker;
      return $('#playlist ul.type-menu').append(item);
    };

    Playlist.prototype.removeMarker = function(markerID) {
      return $('#playlist .type-menu-item').each(function(item) {
        if (item.attr('data-markerID') === markerID) {
          return item.remove();
        }
      });
    };

    Playlist.prototype.setSelectedMarker = function(marker) {
      var m;
      $('#playlist .type-menu-item.selected').removeClass('selected');
      marker.addClass('selected');
      m = this.markers[marker.attr('data-markerID')];
      this.map.map.panTo(m.getPosition());
      if (this.map.currentOpenedInfoWindow != null) {
        this.map.currentOpenedInfoWindow.close();
      }
      return m["infoWindow"].open();
    };

    Playlist.prototype.shiftCurrentMarkerToLeft = function() {
      var marker, selectedItem;
      selectedItem = $('#playlist .type-menu-item.selected');
      marker = selectedItem.prev().length > 0 ? selectedItem.prev() : $('#playlist .type-menu-item').last();
      if (marker.length > 0) {
        return this.setSelectedMarker(marker);
      }
    };

    Playlist.prototype.shiftCurrentMarkerToRight = function() {
      var marker, selectedItem;
      selectedItem = $('#playlist .type-menu-item.selected');
      marker = selectedItem.next().length > 0 ? selectedItem.next() : $('#playlist .type-menu-item').first();
      if (marker.length > 0) {
        return this.setSelectedMarker(marker);
      }
    };
  ...
  CustomMap = (function() {
    ...
    CustomMap.prototype.addMarker = function(markerInfo, otherInfo, isNew, defaultValue) {
      ...
      google.maps.event.addListener(marker, 'click', function(e) {
        _this.playlist.addMarker(marker["cat"], marker["type"], marker.__gm_id);
        if (marker["infoWindow"] != null) {
    ...
    CustomMap.prototype.addMenuIcons = function(callback) {
      var _this = this;
      return this.templateLoader.getTemplate("markersOptions", function(e) {
